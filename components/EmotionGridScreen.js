...
export default function EmotionGridScreen({ title, emotions, onBack, onNext }) {
  const [selectedId, setSelectedId] = useState(null);
  const [frameWidth, setFrameWidth] = useState(REFERENCE_WIDTH);
  const scale = frameWidth / REFERENCE_WIDTH;

  const selectedEmotion = emotions.find((item) => item.id === selectedId) ?? null;
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <BackButton onPress={onBack} arrowStyle={styles.backArrow} />
      </View>
      ...
      <Text style={[styles.title, { top: 156 * scale }]}>{title}</Text>
